...
EvoAgent 的代码在 `agent-store` 仓库的 `community/EvoAgent` 子目录下，`build.sh` 会自动处理克隆逻辑，也可以手动克隆。
...
| `EVOAGENT_STORE_REPO` | `https://gitcode.com/openJiuwen/agent-store.git` | agent-store 地址 |
